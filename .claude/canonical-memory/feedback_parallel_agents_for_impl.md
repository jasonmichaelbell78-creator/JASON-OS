...
- **Why:** Single-threaded implementation leads to "implementation theater" —
  changes that look done but are not functional. Parallel agents with
  convergence loops catch gaps.
- **Apply:** When executing action items or skill updates: design as
  parallelizable tasks, spawn implementation agents, spawn verification
